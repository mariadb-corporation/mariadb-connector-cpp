...
| **`allowMultiQueries`** |Permits sending of multiple statements in a single query, separated by a semicolon, e.g. insert into ab (i) values (1); insert into ab (i) values (2). Is also implicitly enabled by the rewriteBatchedStatements option.|*bool* |false|CLIENT_MULTI_STATEMENTS|
| **`useAffectedRows`** |If true, use "affected rows" for the row count of statements. If false, use "found rows", which corresponds to the JDBC standard. This changes the behavior of, for example, UPDATE... ON DUPLICATE KEY statements.|*bool* |true|CLIENT_FOUND_ROWS|
| **`interactiveClient`** |Session timeout is defined by the wait_timeout server variable. Setting interactiveClient to true will tell the server to use the interactive_timeout server variable.|*bool* |false|CLIENT_INTERACTIVE|
