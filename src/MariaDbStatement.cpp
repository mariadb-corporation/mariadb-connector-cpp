...
namespace mariadb
{
  ...
  bool MariaDbStatement::executeInternal(const SQLString& sql, int32_t fetchSize, int32_t autoGeneratedKeys)
  {
    ...
    try {
      ...
      results.reset(new Results(
            ...
            sql,
            nullptr));
      SQLString nativeSql, timeoutSql;
      protocol->executeQuery(protocol->isMasterConnection(), results.get(), getTimeoutSql(Utils::nativeSql(sql, nativeSql, protocol.get()), timeoutSql));
  ...
  const SQLString& MariaDbStatement::getTimeoutSql(const SQLString& sql, SQLString& buffer)
  {
    if (queryTimeout > 0 && canUseServerTimeout) {
      std::string &buffStr= StringImp::get(buffer);
      const std::string &sqlStr= StringImp::get(buffer);
      constexpr std::size_t setStmtLen= sizeof("SET STATEMENT max_statement_time=") - 1,
        forLen= 5/* FOR */,
        additionalLen= setStmtLen + 10/*int32_t max str len*/ + forLen;
      buffStr.reserve(sqlStr.size() + additionalLen);
      buffStr.append("SET STATEMENT max_statement_time=", setStmtLen).append(std::to_string(queryTimeout)).
        append(" FOR ", forLen).append(sqlStr);
      return buffer;
    }
    return sql;
  }
  ...
  bool MariaDbStatement::testExecute(const SQLString& sql, const Charset& charset)
  {
    ...
    try {
      ...
      protocol->executeQuery(
          ...
          results.get(),
          getTimeoutSql(Utils::nativeSql(sql, nativeSql, protocol.get()), timeoutSql),
          &charset);
