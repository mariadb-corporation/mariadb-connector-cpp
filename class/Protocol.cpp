...
namespace mariadb
{
  ...
  // Callback for C/C for the reconnect event
  void takeCareOfReconnect(void* connect, enum enum_mariadb_status_info type, ...)
  {
    if (type == CLIENT_EVENT_TYPE) {
      ...
      Protocol* protocol= static_cast<Protocol*>(connect);
    ...
    if (options->rewriteBatchedStatements) {
      if (prepareResult->isQueryMultiValuesRewritable()
        ...
        if (options->useBulkStmts
            ...
            && (results->getAutoGeneratedKeys()==Statement::NO_GENERATED_KEYS || (serverCapabilities & MariaDbServerCapabilities::_MARIADB_CLIENT_BULK_UNIT_RESULTS))
  ...
  */
      ...
      bool hasLongData)
  {
    ...
    if (options->useBulkStmts
        && !hasLongData
        && (results->getAutoGeneratedKeys()==Statement::NO_GENERATED_KEYS ||
            (serverCapabilities & MariaDbServerCapabilities::_MARIADB_CLIENT_BULK_UNIT_RESULTS)
