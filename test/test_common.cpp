...
/* {{{ */
static bool populate_test_table(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  std::unique_ptr<sql::Statement> stmt(conn->createStatement());
  ensure("stmt is NULL", stmt.get() != NULL);
...
/* {{{ */
static bool populate_TX_test_table(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  std::unique_ptr<sql::Statement> stmt(conn->createStatement());
  ensure("stmt is NULL", stmt.get() != NULL);
...
/* {{{ */
static bool populate_test_table_PS(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
  ensure("stmt1 is NULL", stmt1.get() != NULL);
  stmt1->execute("USE " + database);

  std::unique_ptr<sql::PreparedStatement> stmt2(conn->prepareStatement("DROP TABLE IF EXISTS test_function"));
  ensure("stmt2 is NULL", stmt2.get() != NULL);
  stmt2->executeUpdate();

  std::unique_ptr<sql::PreparedStatement> stmt3(conn->prepareStatement("CREATE TABLE test_function(a integer unsigned not null, b integer, c integer default null, d char(10), e varchar(10) character set utf8 collate utf8_bin)"));
  ensure("stmt3 is NULL", stmt3.get() != NULL);
  stmt3->executeUpdate();

  std::unique_ptr<sql::PreparedStatement> stmt4(conn->prepareStatement("INSERT INTO test_function (a,b,c,d,e) VALUES(1, 111, NULL, '222', 'xyz')"));
...
/* {{{ */
static bool populate_TX_test_table_PS(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
  ensure("stmt is NULL", stmt1.get() != NULL);
  stmt1->execute("USE " + database);

  std::unique_ptr<sql::PreparedStatement> stmt2(conn->prepareStatement("DROP TABLE IF EXISTS test_function_tx"));
  ensure("stmt2 is NULL", stmt2.get() != NULL);
  stmt2->executeUpdate();

  std::unique_ptr<sql::PreparedStatement> stmt3(conn->prepareStatement("CREATE TABLE test_function_tx(a integer unsigned not null, b integer, c integer default null, d char(10), e varchar(10) character set utf8 collate utf8_bin) engine = innodb"));
  ensure("stmt3 is NULL", stmt3.get() != NULL);
  stmt3->executeUpdate();

  std::unique_ptr<sql::PreparedStatement> stmt4(conn->prepareStatement("INSERT INTO test_function_tx (a,b,c,d,e) VALUES(1, 111, NULL, '222', 'xyz')"));
  ensure("stmt4 is NULL", stmt4.get() != NULL);
  stmt4->executeUpdate();
...
/* {{{ */
static bool populate_test_table_PS_integers(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
  ensure("stmt1 is NULL", stmt1.get() != NULL);
  stmt1->execute("USE " + database);

  std::unique_ptr<sql::PreparedStatement> stmt2(conn->prepareStatement("DROP TABLE IF EXISTS test_function_int"));
  ensure("stmt2 is NULL", stmt2.get() != NULL);
  stmt2->executeUpdate();

  std::unique_ptr<sql::PreparedStatement> stmt3(conn->prepareStatement("CREATE TABLE test_function_int(i integer, i_uns integer unsigned, b bigint, b_uns bigint unsigned)"));
  ensure("stmt3 is NULL", stmt3.get() != NULL);
  stmt3->executeUpdate();
  ...
  try {
    ...
    try {
      ...
      */
      ...
      std::cout << "System error occured: " << se.what() << '(' << se.code() << ')' << std::endl;
    }
    try {
...
/* {{{ */
static void test_connection_1(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
    ...
    std::unique_ptr<sql::Savepoint> savepoint(conn->setSavepoint(savepointName));
...
/* {{{ */
static void test_connection_2(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
/* {{{ */
static void test_connection_3(std::unique_ptr<sql::Connection> & conn, std::string user)
{
  ENTER_FUNCTION();
  try {
...
static void test_statement_1(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_2(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_3(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_4(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_5(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_6(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_7(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_8(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_9(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
...
static void test_statement_10(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt(conn->createStatement());
    ...
    try {
      ensure_equal_str("Table name differs", meta->getTableName(1), std::string("test_function"));
      ensure("Exception not correctly thrown", true/*false*/); //TODO: check if metadata should not return anything, if rs is closed
    } catch (sql::SQLException &/*e*/) {
...
static void test_result_set_0(std::unique_ptr<sql::Connection> & conn)
{
  ...
  try {
    ...
    std::unique_ptr<sql::ResultSet> result(stmt->setResultSetType(sql::ResultSet::TYPE_SCROLL_INSENSITIVE)->executeQuery("SELECT 1, 2, 3"));
...
/* {{{ */
static void test_result_set_2(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
...
/* {{{ */
static void test_result_set_3(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
...
static void test_result_set_4(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
    ...
    stmt1->getConnection()->rollback();
    std::unique_ptr<sql::ResultSet> rset5(stmt1->executeQuery("SELECT COUNT(*) FROM test_function_tx"));
...
static void test_result_set_5(std::unique_ptr<sql::Connection> & conn)
{
  ...
  try {
    ...
    try {
      std::unique_ptr<sql::ResultSet> rset1(stmt1->executeQuery("SELECT COUNT(*) FROM test_function_tx; DELETE FROM test_function_tx"));
...
static void test_result_set_6(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
    ...
    std::unique_ptr<sql::ResultSet> rset1(stmt1->executeQuery("SELECT COUNT(*) AS 'count of rows' FROM test_function_tx"));
    ensure("res1 is NULL", rset1.get() != NULL);
...
static void test_result_set_7(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    ensure("Data not populated", true == populate_TX_test_table(conn, database));

    std::unique_ptr<sql::PreparedStatement> stmt1(conn->prepareStatement("SELECT COUNT(*) AS 'count of rows' FROM test_function_tx"));
    ensure("stmt1 is NULL", stmt1.get() != NULL);
    ensure_equal("sql::Connection differs", conn.get(), stmt1->getConnection());
...
static void test_result_set_8(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    ...
    std::unique_ptr<sql::PreparedStatement> stmt1(conn->prepareStatement("SELECT COUNT(*) FROM test_function_tx"));
    std::unique_ptr<sql::ResultSet> rset1(stmt1->executeQuery());
    ensure("res1 is NULL", rset1.get() != NULL);
    ensure_equal_int("res1 is empty", rset1->next(), true);
    ...
    std::unique_ptr<sql::PreparedStatement> stmt2(conn->prepareStatement("DELETE FROM test_function_tx WHERE 1"));
    /* Let's delete and then rollback */
    ensure_equal_int("Deleted less rows", stmt2->executeUpdate(), count_full_before);

    std::unique_ptr<sql::PreparedStatement> stmt3(conn->prepareStatement("SELECT COUNT(*) FROM test_function_tx"));
    ...
    std::unique_ptr<sql::PreparedStatement> stmt4(conn->prepareStatement("SELECT COUNT(*) FROM test_function_tx"));
    ...
    std::unique_ptr<sql::PreparedStatement> stmt5(conn->prepareStatement("DELETE FROM test_function_tx WHERE 1"));
    /* Let's delete and then rollback */
    ...
    std::unique_ptr<sql::PreparedStatement> stmt6(conn->prepareStatement("SELECT COUNT(*) FROM test_function_tx"));
    ...
    std::unique_ptr<sql::PreparedStatement> stmt7(conn->prepareStatement("DELETE FROM test_function_tx WHERE 1"));
    /* Let's delete and then rollback */
    ensure("Deleted less rows", stmt7->executeUpdate() == count_full_before);
    /* In autocommit iff, this is an op */
    stmt1->getConnection()->rollback();

    std::unique_ptr<sql::PreparedStatement> stmt8(conn->prepareStatement("SELECT COUNT(*) FROM test_function_tx"));
...
static void test_result_set_9(std::unique_ptr<sql::Connection> & conn)
{
  ...
  try {
    ...
    try {
      std::unique_ptr<sql::PreparedStatement> stmt1(conn->prepareStatement("SELECT COUNT(*) FROM test_function_tx; DELETE FROM test_function_tx"));
      ensure("ERR: Exception not thrown", false);
    } catch (sql::SQLException &) {}
  } catch (sql::SQLException &e) {
    printf("\n# ERR: Caught sql::SQLException at %s::%d  [%s] (%d/%s)\n", CPPCONN_FUNC, __LINE__, e.what(), e.getErrorCode(), e.getSQLStateCStr());
...
static void test_result_set_10(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
...
/* {{{ getMetadata() */
static void test_result_set_11(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::Statement> stmt1(conn->createStatement());
    ...
    std::unique_ptr<sql::ResultSet> rset1(stmt1->executeQuery("SELECT * FROM test_function"));
    ensure("res1 is NULL", rset1.get() != NULL);
    ensure("res1 is empty", rset1->next() != false);
...
static void test_prep_statement_0(std::unique_ptr<sql::Connection> & conn)
{
  ...
  try {
    ...
    try {
      std::unique_ptr<sql::PreparedStatement> stmt(conn->prepareStatement("SELECT ?"));
    } catch (sql::SQLException &e) {
      printf("\n# ERR: Caught sql::SQLException at %s::%d  [%s] (%d/%s)\n", CPPCONN_FUNC, __LINE__, e.what(), e.getErrorCode(), e.getSQLStateCStr());
      printf("# ");
      ++total_errors;
    ...
    {
      std::unique_ptr<sql::PreparedStatement> stmt(conn->prepareStatement("SELECT ?, ?, ?, NULL"));
      try {
        stmt->setInt(3, 42);
        stmt->setString(1, "Hello MYSQL");
...
//				ensure("Incorrect value for col 2", !rset->getString(3).compare("42") && false == rset->wasNull());
        ...
        long double col2= rset->getDouble(2);
        printf("# Column 2 value(long double) is: %Lf\n", col2);
        ensure("Incorrect value for col 3", col2 == 1.25 && false == rset->wasNull());
      } catch (sql::SQLException &e) {
        printf("\n# ERR: Caught sql::SQLException at %s::%d  [%s] (%d/%s)\n", CPPCONN_FUNC, __LINE__, e.what(), e.getErrorCode(), e.getSQLStateCStr());
        printf("# ");
        ++total_errors;
...
/* {{{ Test simple update statement against statement object */
static void test_prep_statement_1(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, const std::string & database)
{
  ENTER_FUNCTION();
  try {
    std::unique_ptr<sql::PreparedStatement> stmt0(conn->prepareStatement("SELECT 1, 2, 3"));
    ensure("stmt0 is NULL", stmt0.get() != NULL);

    ensure("Data not populated", true == populate_test_table_PS(conn, database));

    std::unique_ptr<sql::PreparedStatement> stmt1(conn->prepareStatement("SELECT * FROM test_function"));
    ensure("stmt1 is NULL", stmt1.get() != NULL);
    ...
    std::unique_ptr<sql::PreparedStatement> stmt3(conn2->prepareStatement("DROP TABLE test_function"));
    ensure("stmt3 is NULL", stmt3.get() != NULL);
    stmt3->execute();
  } catch (sql::SQLException &e) {
    printf("\n# ERR: Caught sql::SQLException at %s::%d  [%s] (%d/%s)\n", CPPCONN_FUNC, __LINE__, e.what(), e.getErrorCode(), e.getSQLStateCStr());
...
/* {{{ Test simple update statement against statement object */
static void test_prep_statement_2(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
...
/* {{{ Test simple update statement against statement object */
static void test_prep_statement_3(std::unique_ptr<sql::Connection> & conn, std::unique_ptr<sql::Connection> & conn2, const std::string & database)
{
  ENTER_FUNCTION();
  try {
    ...
    std::unique_ptr<sql::PreparedStatement> stmt1(conn->prepareStatement("INSERT INTO test_function_int (i, i_uns, b, b_uns) VALUES(?,?,?,?)"));
    ...
    {
      std::unique_ptr<sql::PreparedStatement> stmt2(conn->prepareStatement("SELECT i, i_uns, b, b_uns FROM test_function_int"));
    ...
    std::unique_ptr<sql::PreparedStatement> stmt5(conn2->prepareStatement("DROP TABLE test_function_int"));
...
/* {{{ Tests blob with PS */
static void test_prep_statement_blob(std::unique_ptr<sql::Connection> & conn, std::string database)
{
  ENTER_FUNCTION();
  try {
...
static void test_not_implemented_connection(std::unique_ptr<sql::Connection> & conn)
{
  ENTER_FUNCTION();
...
static void test_not_implemented_statement(std::unique_ptr<sql::Connection> & conn, const std::string & database)
{
  ENTER_FUNCTION();
  ...
  try {
    ...
    } catch (sql::SQLFeatureNotSupportedException&) {}
...
static void test_not_implemented_conn_meta(std::unique_ptr<sql::Connection> & conn)
{
  ENTER_FUNCTION();
...
static void test_not_implemented_ps(std::unique_ptr<sql::Connection> & conn, const std::string & database)
{
  ENTER_FUNCTION();
...
static void test_not_implemented_resultset(std::unique_ptr<sql::Connection> & conn)
{
  ENTER_FUNCTION();
...
static void test_not_implemented_ps_resultset(std::unique_ptr<sql::Connection> & conn)
{
  ENTER_FUNCTION();

  std::string bar("foo");
  std::unique_ptr<sql::PreparedStatement> stmt(conn->prepareStatement("SELECT 1 AS 'a'"));
...
static void test_not_implemented_cs_resultset(std::unique_ptr<sql::Connection> & conn)
{
  ENTER_FUNCTION();
...
static void test_not_implemented_rs_meta(std::unique_ptr<sql::Connection> & conn)
{
  ENTER_FUNCTION();
...
static void test_not_implemented_cs_rs_meta(std::unique_ptr<sql::Connection> & conn)
{
  ENTER_FUNCTION();

  std::string bar("foo");
  sql::DatabaseMetaData * conn_meta = conn->getMetaData();
  std::unique_ptr<sql::ResultSet> res(conn_meta->getTableTypes());
